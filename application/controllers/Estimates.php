...
class Estimates extends MY_Controller
{
    ...
    function modal_form()
    {
        ...
        $view_data['project_dropdown'] = $this->_get_all_projects_dropdown_list();

        $project_members = $this->Project_members_model->get_project_members_dropdown_list()->result();
    ...
    function save()
    {
        ...
        $estimate_data = array(
            ...
            "project_id" => $this->input->post('project_id'),
            "assign_id" => $this->input->post('assigned_to'),
        );
    ...
    private function _make_row($data)
    {
        ...
        return array(
            "<input type='checkbox' name='checkboxes[]' form='multiple-delete-form' value='".$data->id."'> ",
            $estimate_url,
    ...
    function view($estimate_id = 0)
    {
        ...
        if ($estimate_id) {
            ...
            $price_item = $this->Price_table_email_model->get_details($options)->result();
            $info_item = $this->Template_email_model->get_details($options)->result();
            if ($estimate_info) {
                $view_data['estimate_info'] = $estimate_info;
                $view_data['price_item'] = $price_item;
                $view_data['info_item'] = $info_item;
    ...
    function item_modal_form()
    {
        ...
        $price=$this->Price_table_email_model->get_details($options)->result();
        $view_data['estimate_id'] = $estimate_id;
        $view_data['price'] = $price;
        $this->load->view('estimates/item_modal_form', $view_data);
    }
    function count_price($price_id,$quantity) {
        $options=array("id"=>$price_id);
        $price=$this->Price_table_email_model->get_details($options)->row();
        $rate_1=$price->kg_45;
        $rate_2=$price->kg_100;
        $rate_3=$price->kg_300;
        $rate_4=$price->kg_500;
        $rate_5=$price->kg_1000;
        $total=0;
        if(0 <$quantity && $quantity <= 45){
            $total = $quantity*$rate_1;
        }else if($quantity <= 100){
            $total = (45*$rate_1 + ($quantity-45)*$rate_2);
        }else if($quantity <= 300){
            $total = (45*$rate_1 + 55*$rate_2 + ($quantity-100)*$rate_3);
        }else if ($quantity <= 500){
            $total = (45*$rate_1 + 55*$rate_2 + 200*$rate_3 + ($quantity-300)*$rate_4);
        }else if ($quantity <= 1000){
            $total = (45*$rate_1 + 55*$rate_2 + 200*$rate_3 + 200*$rate_4 + ($quantity-500)*$rate_5);
        }else {
            return false;
        }
        return $total;
    }
    function total_price($rate_price,$estimate_id){
        $options=array("estimate_id"=>$estimate_id);
        $info=$this->Template_email_model->get_details($options)->row();
        $price_1=$info->awb_fee;
        $price_2=$info->customs_export;
        $price_3=$info->airport_handing_fee;
        $price_4=$info->fumigation_fee;
        $price_5=$info->airport_terminal_fee;
        $price_6=$info->pick_up;
        $total_price =$rate_price+$price_1 + $price_2+ $price_3+ $price_4 +$price_5+$price_6;
        return $total_price;
    }
    ...
    function save_item()
    {
        ...
        $total = $this ->count_price($price_id,$quantity);
//        $total = $this ->total_price($rate_price,$estimate_id);
    ...
    private function _make_item_row($data)
    {
        ...
        $rate_name = $this->Price_table_email_model->get_details(array("id" => $rate_id))->row();
        $rate_tile= "<b class='rate'>$rate_name->a_l</b>";
...
//            $parser_data["CONTACT_LAST_NAME"] = $contact_last_name;
            ...
            $email_template = $this->Email_templates_model->get_final_template("send_estimate");
...
//            $view_data['message'] = $this->parser->parse_string($email_template->message, $parser_data, TRUE);
            $view_data["CONTACT_FIRST_NAME"] = $contact_first_name;
            $view_data["CONTACT_LAST_NAME"] = $contact_last_name;
            $view_data['subject'] = $email_template->subject;
            $view_data['price_table']=$this->Price_table_email_model->get_details($options)->result();
            $view_data['info_mail']=$this->Template_email_model->get_details($options)->row();
