...
class Projects extends MY_Controller {
    ...
    function all_tasks_kanban_data() {
        ...
        $statuses = $this->Task_status_model->get_status($options);
    ...
    function task_view() {
        ...
        if($status_id==12) {
            $view_data['info'] = $this->Table_nhan_agent_model->get_details($options)->result();
        }
        if($status_id==13) {
            $view_data['info'] = $this->Table_thubao_nhanhang_model->get_details($options)->result();
        }
        if ($status_id==20) {
        ...
        if ($status_id==18) {
            $view_data['info'] = $this->Nhan_chung_tu_agent_nhap_sea_model->get_details($options)->result();
        }
        if ($status_id==19) {
            $view_data['info'] = $this->Table_hang_tau_thong_bao_model->get_details($options)->result();
        }
    ...
    function my_tasks_list_data() {
        ...
        $status = $this->input->post('status_id') ? implode(",", $this->input->post('status_id')) : "";
        $project_id = $this->input->post('project_id');
        $xuat_air = $this->input->post('xuat_air');
    ...
    function add_tasks(){
            ...
            $view_data['milestones_dropdown'] = array(0 => "-") + $this->Milestones_model->get_dropdown_list(array("title"), "id");
            $project_members = $this->Project_members_model->get_project_members_dropdown_list($project_id)->result();
            $project_members_dropdown = array("" => "-");
            $collaborators_dropdown = array();
            foreach ($project_members as $member) {
            ...
            $this->load->view('projects/add_tasks', $view_data);
    }
     function save_autotask() {
        ...
        $assigned_to = $this->input->post('assign_id');
        $collaborators = $this->input->post('collaborators');
        ...
        function load_ajax()
    {
        ...
        $data =[];
        $data['user_id'] = "";
        $data['member_name'] = '-----select-----';
        $project_members = $this->Project_members_model->get_project_members_dropdown_list($project_id)->result();
        array_unshift($project_members ,$data);
        echo json_encode($project_members);
    }
    function get_project() {
        ...
        $data1['id'] = "";
        $data1['title'] = '-----select-----';
        $data  = $this->Projects_model->get_project_model($client_id);
    ...
    function air_kanban() {
        ...
        $this->template->rander("projects/tasks/kanban/air_tasks", $view_data);
    }
    function air_kanban_data() {
        ...
        $this->load->view('projects/tasks/kanban/air_view', $view_data);
    }
    function air_tasks_kanban_data($project_id = 0) {
        $this->init_project_permission_checker($project_id);
        $style_id = 1;
        if (!$this->can_view_tasks($project_id)) {
            redirect("forbidden");
    ...
    function nhap_air_kanban() {
        ...
        $this->template->rander("projects/tasks/kanban/nhap_air_tasks", $view_data);
    }
    function nhap_air_kanban_data() {
        ...
        $this->load->view('projects/tasks/kanban/nhap_air_view', $view_data);
    }
    function nhap_air_tasks_kanban_data($project_id = 0) {
        $this->init_project_permission_checker($project_id);
        if (!$this->can_view_tasks($project_id)) {
            redirect("forbidden");
        }
    ...
    function nhap_sea_kanban() {
        ...
        $this->template->rander("projects/tasks/kanban/nhap_sea_tasks", $view_data);
    }
    function nhap_sea_kanban_data() {
        ...
        $this->load->view('projects/tasks/kanban/nhap_sea_view', $view_data);
    }
    function nhap_sea_tasks_kanban_data($project_id = 0) {
        $this->init_project_permission_checker($project_id);
        if (!$this->can_view_tasks($project_id)) {
            redirect("forbidden");
        }
