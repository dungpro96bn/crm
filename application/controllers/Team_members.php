...
class Team_members extends MY_Controller {
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                $view_data['show_sender_mail'] = false;
                ...
                if ($this->login_user->is_admin || $user_info->id === $this->login_user->id) {
                    ...
                    $view_data['show_sender_mail'] = true;
                } else {
     ...
     * @param $user_id
     */
    function sender_address($user_id) {
        $this->only_admin_or_own($user_id);
        $view_data['user_info'] = $this->Users_model->get_one($user_id);
        if ($view_data['user_info']->is_admin) {
            $view_data['user_info']->role_id = "admin";
        ...
        $view_data['role_dropdown'] = $this->_get_roles_dropdown();
        $this->load->view("users/sender_address", $view_data);
    }

    // save sender mail
    function save_sender_address($user_id)
    {
        $this->only_admin_or_own($user_id);

        if ($this->input->post('send_test_mail_to2')) {
            $account_data['send_test_mail_to2'] = $this->input->post('send_test_mail_to2');
        }
            $test_email_to = $this->input->post("send_test_mail_to2");
            if ($test_email_to) {
                $account_data = Array(
                    'charset' => 'utf-8',
                    'mailtype' => 'html'
            ...
            if ($this->input->post("email_protocol2") === "smtp") {
                    $account_data['email_protocol2'] = 'smtp';
                if ($this->input->post('imap_host')) {
                    $account_data['imap_host'] = $this->input->post('imap_host');
                }else{$account_data['imap_host'] = "";}
                    if ($this->input->post('email_smtp_host2')) {
                        $account_data['email_smtp_host2'] = $this->input->post('email_smtp_host2');
                    }
                    if ($this->input->post('email_smtp_port2')) {
                        $account_data['email_smtp_port2'] = $this->input->post('email_smtp_port2');
                    }
                    if ($this->input->post('email_smtp_user2')) {
                        $account_data['email_smtp_user2'] = $this->input->post('email_smtp_user2');
                    }
                    if ($this->input->post('email_smtp_pass2')) {
                        $account_data['email_smtp_pass2'] = $this->input->post('email_smtp_pass2');
                    }
                    if ($this->input->post('email_smtp_security_type2')) {
                        $account_data['email_smtp_security_type2'] = $this->input->post('email_smtp_security_type2');
                    }
            }else {
                    $account_data['email_protocol2'] = "";
                }

                $this->load->library('email', $account_data);
                $this->email->set_newline("\r\n");
                if ($this->input->post('email_sent_from_address2')) {
                    $account_data['email_sent_from_address2'] = $this->input->post('email_sent_from_address2');
                }
                if ($this->input->post('email_sent_from_name2')) {
                    $account_data['email_sent_from_name2'] = $this->input->post('email_sent_from_name2');
                }
                $this->email->from($account_data['email_sent_from_address2'], $account_data['email_sent_from_name2']);
