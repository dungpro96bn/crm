...
if (!function_exists('send_app_mail')) {
    ...
    function send_app_mail($to, $subject, $message, $optoins = array()) {
        ...
        if ($ci->Users_model->getSenderProtocol() === "smtp") {
            $email_config["protocol"] = $ci->Users_model->getSenderProtocol();
            $email_config["smtp_host"] = $ci->Users_model->getSenderHost();
            $email_config["smtp_port"] = $ci->Users_model->getSenderPort();
            $email_config["smtp_user"] = $ci->Users_model->getSenderUser();
            $email_config["smtp_pass"] = $ci->Users_model->getSenderPass();
            $email_config["smtp_crypto"] = $ci->Users_model->getSenderSecurityType();
        ...
        $ci->email->from($ci->Users_model->getSenderAddress(),$ci->Users_model->getSenderName());
        $ci->email->to($to);
...
if (!function_exists('get_estimate_status_label')) {
    ...
    function get_estimate_status_label($estimate_info, $return_html = true) {
        ...
        if ($estimate_info->status != "draft" && $estimate_info->due_date < $now && $estimate_info->payment_received < $estimate_info->estimate_value) {
            $estimate_status_class = "label-danger";
            $status = "overdue";
        } else if ($estimate_info->status !== "draft" && $estimate_info->payment_received <= 0) {
            ...
            $status = "partially_paid";
        } else if ($estimate_info->status === "draft") {
            $estimate_status_class = "label-default";
            $status = "draft";
        }
