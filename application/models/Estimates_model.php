...
class Estimates_model extends Crud_model {
    ...
    function get_estimate_total_summary($estimate_id = 0) {
        ...
        $clients_table = $this->db->dbprefix('clients');
        $taxes_table = $this->db->dbprefix('taxes');
        $price_table = $this->db->dbprefix('tempalte_email');
        ...
        $info_price_sql = "SELECT SUM($price_table.awb_fee + $price_table.airport_terminal_fee + $price_table.customs_export + $price_table.airport_handing_fee + $price_table.fumigation_fee + $price_table.pick_up) AS sub_price FROM $price_table WHERE $price_table.estimate_id=$estimate_id";
        $sub_price = $this->db->query($info_price_sql)->row();


        $result = new stdClass();
        $result->sub_price=$sub_price->sub_price;
        $result->estimate_subtotal = (($item->estimate_subtotal)+($sub_price->sub_price));
        ...
        $result->estimate_total = $result->estimate_subtotal + $result->tax + $result->tax2;
