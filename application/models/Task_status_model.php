...
class Task_status_model extends Crud_model {
    ...
    function get_status($options = array()) {
        $task_status_table = $this->db->dbprefix('task_status');
        $styles = get_array_value($options, "style");
        $where = "";
        $id = get_array_value($options, "id");
        $status = get_array_value($options, "statuss");
        if ($status) {
            $where = " AND $task_status_table.status=$status";
        }
        if ($id) {
            $where = " AND $task_status_table.id=$id";
        }
        if ($styles) {
            $where = " AND $task_status_table.style=$styles";
        }
