...
class Users_model extends Crud_model
{
    ...
    function getSenderAddress(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_sent_from_address2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_sent_from_address2');
    }
    function getSenderName(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_sent_from_name2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_sent_from_name2');
    }
    function getSenderProtocol(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_protocol2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_protocol2');
    }
    function getSenderHost(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_smtp_host2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_smtp_host2');
    }
    function getSenderUser(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_smtp_user2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_smtp_user2');
    }
    function getSenderPass(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_smtp_pass2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_smtp_pass2');
    }
    function getSenderPort(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_smtp_port2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_smtp_port2');
    }
    function getSenderSecurityType(){
        $users_table = $this->db->dbprefix('users');
        $sql = "SELECT $users_table.email_smtp_security_type2
        FROM $users_table
        WHERE $users_table.id = ". self::login_user_id();
        return $this->db->query($sql)->row('email_smtp_security_type2');
    }
