...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("body").on("click", "tr", function () {
            ...
            if (!$(this).hasClass("active")) {
                var message_id = $(this).find(".message-row").attr("data-id"),
                      ...
                      email_number =  $(this).closest('tr').find("input").val();
                if (message_id) {
                    ...
                    $(this).addClass("active");
                $.ajax({
                    url: "<?php echo get_uri("messages/ajax_reader"); ?>/" + email_number,
                    type:'POST',
                    success: function (data) {
                        if (data) {
                            $("#message-details-section").html(data);
                            note();
                            delete_mail();
                        } else {
        ...
        function note() {
            $("#reply_mail").appForm({
                onSuccess: function(result) {
                    ...
                    $(result.data).insertBefore("#reply-form");
                   $("#reply_message_mail").val("");
                    $("#ajaxModal").remove();
                }
            });
        }
        ...
        $("#delete_mail").on("click",function () {
            ...
            $.ajax({
                url: "<?php echo get_uri("messages/delete_mail"); ?>/" + email_number,
                type:'get',
                success: function (result) {
